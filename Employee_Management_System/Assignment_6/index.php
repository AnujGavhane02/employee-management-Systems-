...
// Resolve employee record linked to logged-in user when needed.
$currentEmployeeId = null;
$employeeLookupStmt = $pdo->prepare('SELECT id FROM employees WHERE user_id = ? LIMIT 1');
$employeeLookupStmt->execute([$userId]);
$currentEmployeeId = $employeeLookupStmt->fetchColumn() ?: null;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'leave_request') {
        ...
        if (!$targetEmployeeId && $userRole === 'admin') {
            $picked = intval($_POST['leave_employee_id'] ?? 0);
            if ($picked > 0) {
                $chk = $pdo->prepare('SELECT id FROM employees WHERE id = ? AND status = "active"');
                $chk->execute([$picked]);
                $targetEmployeeId = $chk->fetchColumn() ? $picked : null;
            }
            if (!$targetEmployeeId) {
                $errors[] = 'Select which employee this leave request is for.';
            }
        } elseif (!$targetEmployeeId) {
            $errors[] = 'No employee profile is linked to your account. Please contact an administrator.';
        }

        if (empty($errors) && (empty($startDate) || empty($endDate) || empty($leaveType))) {
            $errors[] = 'All fields are required.';
        } elseif (empty($errors) && strtotime($startDate) > strtotime($endDate)) {
            ...
            $stmt->execute([$targetEmployeeId, $leaveType, $startDate, $endDate, $days, $reason]);
            $successMessage = 'Leave request submitted successfully.';
        }
    }

    // Leave approval/rejection (admin only)
    if (($action === 'approve_leave' || $action === 'reject_leave') && $userRole === 'admin') {
        $leaveId = intval($_POST['leave_id'] ?? 0);
        if ($leaveId <= 0) {
            $errors[] = 'Invalid leave request selected.';
        } else {
            ...
            $stmt->execute([$newStatus, $userId, $leaveId]);
            $successMessage = $action === 'approve_leave'
...
foreach ($params as $idx => $param) {
    $stmt->bindValue($idx + 1, $param, PDO::PARAM_STR);
}
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <form method="POST">
        ...
        <div class="modal-body">
          <?php if ($userRole === 'admin' && !$currentEmployeeId): ?>
          <div class="mb-3">
            <label class="form-label">Employee *</label>
            <select name="leave_employee_id" class="form-select" required>
              <option value="">Select employee</option>
              <?php
              $leaveEmpStmt = $pdo->query('SELECT id, CONCAT(first_name, " ", last_name) AS name FROM employees WHERE status = "active" ORDER BY first_name, last_name');
              while ($le = $leaveEmpStmt->fetch(PDO::FETCH_ASSOC)) {
                  echo '<option value="' . (int)$le['id'] . '">' . htmlspecialchars($le['name']) . '</option>';
              }
              ?>
            </select>
            <small class="text-muted">Your login is not linked to an employee record; pick who this leave is for.</small>
          </div>
          <?php endif; ?>
          <div class="mb-3">
